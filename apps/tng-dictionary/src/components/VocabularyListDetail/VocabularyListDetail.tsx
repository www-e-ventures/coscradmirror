...
const filterEntriesForSelectedTerms = (
    allEntries: VocabularyListEntry[],
    filters: Record<string, string | boolean>
): TermData[] =>
    allEntries
        .filter(({ variableValues }) => doValuesMatchFilters(variableValues, filters))
        .map(({ term }) => term);
...
export function VocabularyListDetail(props: VocabularyListDetailProps) {
    ...
    useEffect(() => {
        ...
        fetch(apiUrl, { mode: 'cors' })
            ...
            .then((vocabularyList) => {
                ...
                setFormState({
                    currentSelections: buildUnselectedFormData(
                        appState.vocabularyList?.variables.map(({ name }) => name)
                    ),
                });
            })
    ...
    const selectedTerms = filterEntriesForSelectedTerms(
        allEntries,
        removeNoSelectionValuedPropsFromFilters(form.currentSelections)
    );
            ...
            <div className="vocabError">
                <section className="vocabHeader">
                    <h1 style={{ lineHeight: '0px' }}>
                        Vocabulary List <InfoTwoToneIcon className="headerIcon" />
                    </h1>
                    <div>
                ...
                <div className="background">
                    <VocabularyListForm
                        formItems={(appState.vocabularyList as unknown as any).variables}
                    />
